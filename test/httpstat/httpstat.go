...
func grayscale(code Attribute) func(string, ...interface{}) string {
	return New(code + 232).SprintfFunc()
}
...
func visit(url *url.URL) {
	...
	trace := &httptrace.ClientTrace{
		...
		ConnectDone: func(net, addr string, err error) {
			...
			printf("\n%s%s\n", GreenString("Connected to "), CyanString(addr))
		},
	...
	printf("\n%s%s%s\n", GreenString("HTTP"), grayscale(14)("/"), CyanString("%d.%d %s", resp.ProtoMajor, resp.ProtoMinor, resp.Status))
	...
	for _, k := range names {
		printf("%s %s\n", grayscale(14)(k+":"), CyanString(strings.Join(resp.Header[k], ",")))
	}
	...
	fmta := func(d time.Duration) string {
		return CyanString("%7dms", int(d/time.Millisecond))
	}

	fmtb := func(d time.Duration) string {
		return CyanString("%-9s", strconv.Itoa(int(d/time.Millisecond))+"ms")
	}
